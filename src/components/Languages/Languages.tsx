...
function Languages(): React.JSX.Element {
  ...
  const [languages, setLanguages]: UseStateType<LanguageDto[]> = useState<
    LanguageDto[]
  >([]);

  useEffect(() => {
    const languagesHttpService: LanguagesHttpService = new LanguagesHttpService(
      configService
    );
  ...
  return (
    <Category
      ...
      loading={loading}
      contentElement={languagesElements()}
    />
  );
}
