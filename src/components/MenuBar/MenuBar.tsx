...
const themeService: ThemeService = new ThemeService(new SessionStorageService());
...
function MenuBar(): React.JSX.Element {
    ...
    let [themeIcon, setThemeIcon]: UseStateType<IconDefinition> = useState(iconsService.faMoonSolid);
    ...
    function minimizedButtonElement() {
        if (!minimized) {
            return (
                <span className={styles.link}>
                    <FontAwesomeIcon icon={iconsService.faMinus} onClick={() => setMinimized(true)}
        ...
        return (
            <span className={styles.link}>
                <FontAwesomeIcon icon={iconsService.faPlus} onClick={() => setMinimized(false)}
                                 className={styles.minimize}/>
            </span>
        );
    }
        ...
        <>
            <nav className={classNames({[styles.MenuBar]: true, [styles.minimized]: minimized})}>
                ...
                <div>
                    ...
                    <span className={styles.link}>
                        <FontAwesomeIcon icon={themeIcon} onClick={nextTheme}
