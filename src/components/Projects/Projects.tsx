...
function Projects(): React.JSX.Element {
  ...
  const [projects, setProjects]: UseStateType<ProjectDto[]> = useState<
    ProjectDto[]
  >([]);

  useEffect(() => {
    const projectsHttpService: ProjectsHttpService = new ProjectsHttpService(
      configService
    );
      ...
      <>
        <div>
          ...
          <span className={styles.teamSize}>
            {messages.teamSizeFormat(project.teamSize)}
          </span>
        </div>
        <div>
  ...
  return (
    <>
      <Category
        ...
        loading={loading}
        contentElement={projectsElements()}
      />
    </>
  );
}
